...
def plot_signal_and_spikes_from_bandpassfilteredsignal(bandpassfilteredsignal,
                                                       spikes, threshold, channellabel, fs, time_in_sec, from_in_s=0,
                                                       to_in_s=None, tick=40,
    ...
    '''
    start_range = (max(0, from_in_s))
    if to_in_s == None:
        stop_range = len(bandpassfilteredsignal) * to_sec
    else:
        stop_range = min(len(bandpassfilteredsignal) * to_sec, to_in_s)
    ...
    signal_in_range = bandpassfilteredsignal[int(range_in_s[0] / to_sec):int(range_in_s[1] / to_sec)]
    ...
    ax = plt.plot(spikes_in_range, [threshold - 1] * spikes_in_range.shape[0], 'ro', ms=2, c="#D9580D")
